            ...
            for file in files:
                if file.endswith(".py"):
                    ...
                    try:
                        ...
                        print(
                            f"⚠️ Überspringe Datei wegen Kodierungsfehler: "
                            f"{filepath}"
                        )
                        continue
                    ...
                    while i < len(lines):
                        line = lines[i]
                        match = re.match(r"#\s*arc42:\s*((?:\d+\.)*\d+)\s+(.*)", line)
                        if match:
                            ...
                            while (
                                ...
                                description_lines.append(lines[i].lstrip("#").strip())
                                i += 1
                            arc_sections.append(
...
def write_markdown(sections, output_file="ARCHITEKTUR.md"):
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for section_id, title, description in sorted(
            sections, key=lambda x: list(map(int, x[0].split(".")))
        ):
